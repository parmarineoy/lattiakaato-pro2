# -*- coding: utf-8 -*-
"""
Lattiakaato PRO2 – FIX2 (step-mode)
Lisäys: UI-valinta korkoeron viitteelle:
- "0-piste (kaivo)" => ulkopuoli >= step + k_o_min * d_edge
- "Suihkualueen reuna" => ulkopuoli >= max(step + h_edge_min, step + k_o_min * d_edge)
"""
import io, math
...
class Config:
    ...
    step_mode: StepMode                # 'drain0' tai 'edge'
        ...
        for i in range(xs.size):
            ...
            for s_idx, sh in enumerate(cfg.showers):
                if inside_rect(x, y, sh.rect):
                    ...
                    lowers.append(lv)
                    controllers.append((lv, f'shower#{s_idx}:inside'))
                else:
                    ...
                    if allow_this_shower:
                        ...
                        if cfg.step_mode == 'edge':
                            # Reunaan sidottu step
                            r_edge = math.hypot(cx - sh.drain_x, cy - sh.drain_y)
                            h_edge_min = max(0.0, cfg.k_s_min * r_edge)
                            base_outside  = step_m + h_edge_min
                            slope_outside = step_m + cfg.k_o_min * d_edge
                            lv = max(base_outside, slope_outside)
                            origin = 'edge'
                        else:
                            # 0-pisteeseen sidottu step (alkuperäinen malli)
                            lv = step_m + cfg.k_o_min * d_edge
                            origin = 'drain0'
                        lowers.append(lv)
                        controllers.append((lv, f'shower#{s_idx}:outside({origin})'))

            # 2) Muut kaivot: pisteohjaus (ei step-lisää)
            for d_idx, (dx, dy) in enumerate(cfg.drains):
                ...
                if allow_this_drain:
                    ...
                    lv = cfg.k_o_min * d
                    lowers.append(lv)
                    controllers.append((lv, f'drain#{d_idx}'))
...
def plot_map(df: pd.DataFrame, cfg: Config, layer='FFL_mm', show_labels=False):
    ...
    ax.set_xlabel('x [m]'); ax.set_ylabel('y [m]'); ax.set_title('Lattiakaatojen korkomalli (step-mode)')
...
def main():
    st.set_page_config(page_title='Lattiakaato PRO2 – FIX2 (step-mode)', layout='wide')
    st.title('🛁 Lattiakaatojen suunnittelutyökalu — PRO2 (step-mode)')
    ...
    with st.sidebar:
        ...
        step_mode_label = st.radio(
            'Korkoeron viite',
            ['0-piste (kaivo)', 'Suihkualueen reuna'],
            index=1,
            help=(
                "Määrittää mihin korkoeron 'step' viitataan ulkopuolella.\n" \
                "• 0-piste (kaivo): ulkopuoli ≥ step + ulkoalueen min-kaato * etäisyys reunasta.\n" \
                "• Suihkualueen reuna: ulkopuoli ≥ step yli reunan sisäpuolen min-korkeuden (turvallisempi)."
            )
        )
    ...
    with c1:
        ...
        st.download_button('⤓ PNG', data=buf.getvalue(), file_name='lattiakaadot_stepmode.png', mime='image/png')
    with c2:
        st.download_button('⤓ CSV', data=df.to_csv(index=False).encode('utf-8'), file_name='korkopisteet_stepmode.csv', mime='text/csv')
